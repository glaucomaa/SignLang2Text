    ...
    for frames, targets in loader:
        ...
        for i in range(B):
            ...
            while ys.size(1) < max_len:
                next_tok = model(clip, ys)[:, -1].argmax(-1, keepdim=True)
...
def main(cfg: DictConfig):
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        best_val = getattr(main, "_best_val", float("inf"))
        ...
        # if val_loss < best_val := getattr(main, "_best_val", float("inf")):
        #     main._best_val = best_val
        #     save_checkpoint(model, best_path)
        #     logger.info(f" ▸ New BEST ckpt saved (val_loss={val_loss:.4f})")
        #     if task:
        #         upload_replace(task, "best_ckpt", best_path)
